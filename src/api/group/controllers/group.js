...
module.exports = createCoreController("api::group.group", ({ strapi }) => ({
  ...
  async inviteLearners(ctx) {
    ...
    // Check if the number of invites/emails is less than or equal to the number of seats available
    if (inviteEmails.length > group?.numberOfSeatsAvailable && !resendInvite) {
      return captureSentryErrorAndReturnBadRequest(
        `Admin group invite Error: The number of invites: [${inviteEmails.length}] for Admin User: [${userId}] is greater than the number of seats left: [${group?.numberOfSeatsAvailable}] for group: ${groupId}`,
        ctx
      );
    }
    ...
    let numberOfSeatsAvailable =
      group.numberOfSeatsAvailable - emailsNotInPendingLearners.length;
  ...
  async removePendingLearners(ctx) {
    ...
    try {
      updateGroupResponse = await strapi
        ...
        .updateGroup(group.id, {
          pendingLearners: newPendingLearners,
          numberOfSeatsAvailable: group.numberOfSeatsAvailable + 1,
        });
    } catch (error) {
      return captureSentryErrorAndReturnBadRequest(
  ...
  async convertPendingLearners(ctx) {
    ...
    if (!convertPendingEmail || !groupId) {
      ...
      return ctx.redirect(
        `${redirectUrlFrontEnd}/error/common.errorCode.groupInviteError`
      );
    }
    ...
    try {
      ...
      if (!isEmailInPending) {
        ...
        captureSentryErrorAndSetJobStatusToFailed(jobId, jsonLogs);
        return ctx.redirect(
          `${redirectUrlFrontEnd}/error/common.errorCode.emailDoesntExistGroupInvitationError`
        );
      }
