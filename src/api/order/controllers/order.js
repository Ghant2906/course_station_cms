...
module.exports = createCoreController("api::order.order", ({ strapi }) => ({
  ...
  async create(ctx) {
    ...
    const { products, locale } = sanitizedBody;
  ...
  /**
   * Get order invoice from Stripe
   */
  async getOrderInvoice(ctx) {
    await this.validateQuery(ctx);
    const sanitizedQueryParams = await this.sanitizeQuery(ctx);
    ...
    if (!orderId) {
      return ctx.badRequest(`OrderId is required to find an order`);
    }
    ...
    if (!sanitizedResults || sanitizedResults.length === 0) {
      return ctx.notFound(
        `Order was not found for orderId: ${orderId} and customer: ${email}`
      );
    }

    const { checkoutSessionId } = sanitizedResults[0];

    try {
      const orderSession = await retrieveStrpieCheckoutSession(
        checkoutSessionId
      );

      const { invoice } = orderSession;

      if (!invoice) {
        captureSentryError(
          new Error(
            `Invoice was not created for orderId: ${orderId} with checkout session id: ${checkoutSessionId}`
          )
        );
        return ctx.notFound(
          `Invoice was not created for orderId: ${orderId} with checkout session id: ${checkoutSessionId}`
      ...
      const invoiceResponse = await retrieveInvoice(invoice);
      const { hosted_invoice_url } = invoiceResponse;
      ...
      return ctx.send({
        invoiceUrl: invoiceResponse,
      });
    } catch (error) {
      captureSentryError(
        new Error(
          `There was an error retrieving a checkout session in Stripe for session Id: ${checkoutSessionId}. Error: ${error.message}`
        )
      );
      return ctx.notFound(
        `There was an error retrieving a checkout session in Stripe for session Id: ${checkoutSessionId}. Error: ${error.message}`
